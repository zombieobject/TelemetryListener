...
//
//
//  Created by Ethan Mateja on 9/2/24.
//
...
public extension UnsafeMutablePointer where Pointee: FixedWidthInteger {
    func compare(with buffer: UnsafeMutablePointer<Pointee>, size: Int) -> Bool {
    ...
    func compare(with buffer: UnsafePointer<Pointee>, size: Int) -> Bool {
        for i in 0..<size where self[i] != buffer[i] {
            return false
        }
